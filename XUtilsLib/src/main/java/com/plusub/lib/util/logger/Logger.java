...
public final class Logger {
    ...
    private static void logChunk(int logType, String tag, String chunk) {
        ...
        //save file log
        if (logType >= settings.fileLogLevel){
            LogFile.log(tag, chunk);
        }
    }
    ...
    public static class Settings {
        ...
        //save log to file level
        int fileLogLevel = 1000;
        ...
        /**
         * 保存日志到文件,默认不保存(保存前提是LogLevel为FULL)
         * 保存的条件是level=Log.VERBOSE的所有级别日志
         * 如设置level=Log.VERBOSE,则会保持所有日志
         * 设置level=Log.ERROR,则会保持Log.ERROR和Log.ASSERT的所有日志
         * 设置level>Log.ASSERT,则不会保持任何日志
         * @param level
         * @return
         */
        public Settings setLogFileLevel(int level){
